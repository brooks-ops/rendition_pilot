...
from app.review_workflow import get_decision_label, stamp_reviewed_pdf, wrap_text
...
def test_get_decision_label_supports_special_statuses():
    assert get_decision_label("accepted") == "ACCEPTED"
    assert get_decision_label("closed") == "CLOSED"
    assert get_decision_label("no_assets") == "NO ASSETS"
...
def test_stamp_reviewed_pdf_marks_closed_status_in_red_review_stamp(tmp_path, monkeypatch):
    ...
    out_path = stamp_reviewed_pdf(
        ...
        final_record={
            "account_number": "P12345",
            "final_value": 0,
            "final_source": "closed_account",
            "appraiser_initials": "BB",
            "decision": "closed",
            "locked_at": "2026-04-29T15:00:00",
            "appraiser_notes": "Business reported closed.",
        },
    )
    ...
    try:
        full_text = "\n".join(page.get_text() for page in stamped)
        assert "STATUS: CLOSED" in full_text
        assert "CLOSED" in full_text
        assert "VALUE: $0.00" in full_text
    finally:
        stamped.close()
