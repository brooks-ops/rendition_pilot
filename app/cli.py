...
def build_cli_summary(result: dict[str, Any], source_path: str | None = None) -> str:
    ...
    lines.append("-" * 78)
    lines.append(f"Section 3 Present:      {format_bool(form_flags.get('section_3_present'))}")
    lines.append(f"Section 3 Box Checked:  {format_bool(form_flags.get('section_3_prior_year_checked'))}")
    ...
    lines.append(f"Under $20k Checked:     {format_bool(form_flags.get('section_5_under_20k_checked'))}")
    lines.append(f"$20k+ Checked:          {format_bool(form_flags.get('section_5_20k_or_more_checked'))}")
    lines.append(f"$125k or Less Checked:  {format_bool(form_flags.get('section_5_125k_or_less_checked'))}")
    lines.append(f"> $125k Checked:        {format_bool(form_flags.get('section_5_more_than_125k_checked'))}")
